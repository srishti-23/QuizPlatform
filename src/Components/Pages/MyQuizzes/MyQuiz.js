...
const MyQuiz = () => {
  ...
  const handleDelete = (id) => {
    //this function will execute when delete icon is clicked
    setDeleteID(id);
    setModal(true);
  };

  const deleteYes = () => {
    //this function will execute when yes button/option is clicked when delete warning Modal is shown
    dispatch(deleteQuiz(deleteID));
    setModal(false);
  };

  const toggleHandler = (id) => {
    //this function will execute when toggle buton is clicked
  ...
  return (
    <div className="whole" >
      <div className="head">
        <h2>MY QUIZZES</h2>
        <Button component={Link} to={"/create-new"} variant="contained" >
          Create New quiz
        </Button>
      </div>
      ...
      {modal === true ? (
        <div className="deleteouter">
          <div className="deleteModal">
            ...
            <div className="delbtn">
              <Button variant="contained" onClick={() => deleteYes()}>
                Yes
              </Button>{" "}
              {/**if yes button is clicked then delete the quiz */}
              <Button
                sx={{ marginLeft: "10px" }}
                variant="contained"
              ...
              {/**if no button is clicked then set the value of madal to false which will close the modal */}
            </div>
          </div>
        </div>
      ) : (
        <div className="all-quiz-container">
          {Quiz.length === 0 ? ( //if there are no quiz then show this message else show the list of quizzes
            <p >
              Oops ! Currently there are no quizzes.
            </p>
          ) : (
            <div className="table">
              <table>
